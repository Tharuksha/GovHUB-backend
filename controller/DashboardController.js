...
class DashboardController {
      ...
      const solvedTicketCounts = await Ticket.aggregate([
        ...
        { $unwind: "$staffDetails" }, // Unwind the staff details array
        {
          $project: {
            ...
            }, // Include the staff name from staffDetails
  ...
  async getSolvedAndPendingTicketCounts(req, res) {
    try {
      ...
      const result = {
        solved:
          ticketCounts.find((ticket) => ticket.status === "Approved")?.count ||
          0,
        pending:
          ticketCounts.find((ticket) => ticket.status === "Pending")?.count ||
          0,
      };
  ...
  async getRecentlySolvedTicketByStaff(req, res) {
    ...
    try {
      ...
      if (recentlySolvedTicket) {
        ...
        res
          .status(404)
          .json({ message: "No solved tickets found for this staff member." });
      }
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
